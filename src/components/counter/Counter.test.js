...
describe('Counter Component', () => {
  ...
  describe('render', () => {
    ...
    test('successfully render component', () => {
      const wrapper = setup();
      const counterComponent = findByDataTestAttribute(wrapper, 'component-counter');
      expect(counterComponent.length).toBe(1);
    });

    test('successfully render counter display', () => {
      const wrapper = setup();
      const counterDisplay = findByDataTestAttribute(wrapper, 'counter-display');
      expect(counterDisplay.length).toBe(1);
    });

    test('successfully render increase counter button', () => {
      const wrapper = setup();
      const counterIncrementButton = findByDataTestAttribute(wrapper, 'counter-increment-button');
      expect(counterIncrementButton.length).toBe(1);
    });

    test('successfully render decrease counter button', () => {
      const wrapper = setup();
      const counterDecrementButton = findByDataTestAttribute(wrapper, 'counter-decrement-button');
    ...
    test('successfully displays error message', () => {
      ...
      const errorDisplay = findByDataTestAttribute(wrapper, 'counter-error');
  ...
  describe('functionality', () => {
    test('counter value increases on clicking increment button', () => {
      ...
      const incrementButton = findByDataTestAttribute(wrapper, 'counter-increment-button');
    ...
    test('counter value decreases on clicking decrement button', () => {
      const counter = 8;
      const wrapper = setup({}, {counter});
      const decrementButton = findByDataTestAttribute(wrapper, 'counter-decrement-button');
